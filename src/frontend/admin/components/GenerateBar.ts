    ...
    return div(
        ...
        () => {
            if (availableTags.val.length === 0) {
                return span({ style: "font-size:12px;color:var(--text-muted);" }, "暂无标签");
            }
            return div(
                { style: "display:flex;flex-wrap:wrap;gap:6px;" },
                    ...
                    button(
                        {
                            class: () =>
                                "mode-btn" +
                                (selectedTagFilter.val === tag ? " active" : ""),
                            style:
    ...
    return div(
        ...
        () => {
            if (prompts.val.length === 0) {
                return span({ style: "font-size:12px;color:var(--text-muted);" }, "暂无提示词");
            }
            return div(
                { style: "display:flex;flex-wrap:wrap;gap:6px;" },
                ...prompts.val.map((p) =>
                    button(
                        {
                            class: () =>
                                "mode-btn" +
                                (selectedPromptId.val === p.id ? " active" : ""),
                            style:
                                "font-size:12px;padding:3px 10px;border-radius:12px;" +
                                "white-space:nowrap;",
                            disabled: () => generating.val,
