...
B --> H["创意内容 API<br/>src/api/creative.ts"]
F --> I["速率限制<br/>src/helper/rate-limit.ts"]
G --> J["配置加载<br/>src/config/app-config.ts"]
A --> K["运行时与脚本<br/>package.json"]
```
...
- API 层
  ...
  - AI API：内容优化、标签建议、统一写作操作、对话式工作台。
  - 创意内容 API：提示词管理、上下文预览、创意内容生成与存储。
- AI 与嵌入
...
- [src/api/creative.ts:27-383](file://src/api/creative.ts#L27-L383)
...
CREA["创意内容 API<br/>src/api/creative.ts"]
END
FE1 --> S
FE2 --> S
S --> APIA
...
- 提示词管理
  - 支持增删改查提示词，用于创意内容生成。
- 上下文预览
  - 支持手动选择备忘录、按标签筛选或自动语义检索三种模式。
- 流式生成
  - 基于 AI 提供商进行流式内容生成，完成后持久化存储。

```mermaid
flowchart TD
Start(["请求进入 /api/creative"]) --> Mode{"上下文模式"}
Mode --> |手动| Manual["使用 memo_ids"]
Mode --> |标签| Tag["按 tag 过滤 memos"]
Mode --> |自动| Auto["语义检索 + 嵌入"]
Manual --> Stream["流式生成内容"]
Tag --> Stream
Auto --> Embed["生成嵌入向量"]
Embed --> Stream
Stream --> Save["保存到数据库"]
...
图表来源
- [package.json:6-12](file://package.json#L6-L12)
- [src/server.ts:119-137](file://src/server.ts#L119-L137)
...
### 六、创意内容生成问题
- 症状
  - 提示词管理失败、上下文预览为空、生成过程中断。
- 诊断步骤
  - 检查提示词表是否初始化；确认上下文模式参数正确。
  - 查看 AI 生成日志与速率限制状态。
- 常见原因与修复
  - 提示词缺失：运行种子数据初始化或手动创建提示词。
  - 上下文为空：检查标签筛选条件或语义检索结果。
  - 生成中断：检查网络连接与 AI 配额；查看 SSE 流错误。
