# 备忘录固定功能设计规格

<cite>
**本文档引用的文件**
- [2026-05-23-memo-pin-feature-design.md](file://docs/superpowers/specs/2026-05-23-memo-pin-feature-design.md)
- [memos.ts](file://src/api/memos.ts)
- [model.ts](file://src/model.ts)
- [MemoCard.ts](file://src/frontend/admin/components/MemoCard.ts)
- [memo.ts](file://src/frontend/admin/actions/memo.ts)
- [db.ts](file://src/db.ts)
- [svgHelper.ts](file://src/helper/svgHelper.ts)
- [index.ts](file://src/frontend/masonry/index.ts)
- [components.ts](file://src/frontend/masonry/components.ts)
- [state.ts](file://src/frontend/masonry/state.ts)
- [api.ts](file://src/frontend/masonry/api.ts)
- [README.md](file://README.md)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构概览](#项目结构概览)
3. [核心组件分析](#核心组件分析)
4. [架构设计](#架构设计)
5. [详细组件分析](#详细组件分析)
6. [数据库设计](#数据库设计)
7. [API 设计](#api-设计)
8. [前端实现](#前端实现)
9. [排序算法](#排序算法)
10. [性能考虑](#性能考虑)
11. [故障排除指南](#故障排除指南)
12. [总结](#总结)

## 简介

备忘录固定功能为 Memos 备忘录系统增加了置顶（Pin）能力，允许管理员将重要的备忘录置顶，使其在列表顶部优先显示。该功能在 Admin 后台和瀑布流公开页均生效，提供直观的状态标记和便捷的操作按钮。

## 项目结构概览

```mermaid
graph TB
subgraph "后端层"
API[API 层<br/>memos.ts]
DB[数据库层<br/>db.ts]
MODEL[数据模型<br/>model.ts]
end
subgraph "前端层"
subgraph "管理后台"
ADMIN[Admin SPA<br/>VanJS]
MEMOCARD[MemoCard 组件<br/>Admin UI]
ACTIONS[Actions<br/>memo.ts]
end
subgraph "瀑布流页面"
MASONRY[Masonry SPA<br/>VanJS]
COMPONENTS[Masonry 组件<br/>components.ts]
STATE[Masonry 状态<br/>state.ts]
APIFRONTEND[Masonry API<br/>api.ts]
end
SVG[SVG 图标<br/>svgHelper.ts]
end
API --> DB
DB --> MODEL
ADMIN --> MEMOCARD
ADMIN --> ACTIONS
MEMOCARD --> SVG
MASONRY --> COMPONENTS
MASONRY --> STATE
MASONRY --> APIFRONTEND
ACTIONS --> API
APIFRONTEND --> API
```

**图表来源**
- [memos.ts:1-220](file://src/api/memos.ts#L1-L220)
- [db.ts:1-479](file://src/db.ts#L1-L479)
- [MemoCard.ts:1-346](file://src/frontend/admin/components/MemoCard.ts#L1-L346)
- [components.ts:1-337](file://src/frontend/masonry/components.ts#L1-L337)

## 核心组件分析

### 数据模型扩展

备忘录数据模型新增了 `pinned_at` 字段，用于存储置顶时间戳：

```mermaid
classDiagram
class Memo {
+number id
+string content
+string[] tags
+boolean is_public
+string|null pinned_at
+string created_at
+string updated_at
}
class MemoRow {
+number id
+string content
+string tags
+number is_public
+string|null pinned_at
+string created_at
+string updated_at
}
class Card {
+number id
+string text
+PreparedText prepared
+string updatedAt
+string|null pinnedAt
+string rawText
}
Memo --> MemoRow : "转换"
Card --> Memo : "使用"
```

**图表来源**
- [model.ts:1-35](file://src/model.ts#L1-L35)
- [db.ts:64-120](file://src/db.ts#L64-L120)
- [state.ts:13-20](file://src/frontend/masonry/state.ts#L13-L20)

**章节来源**
- [model.ts:1-35](file://src/model.ts#L1-L35)
- [db.ts:64-120](file://src/db.ts#L64-L120)
- [state.ts:13-20](file://src/frontend/masonry/state.ts#L13-L20)

## 架构设计

备忘录固定功能采用分层架构设计，确保前后端分离和数据一致性：

```mermaid
sequenceDiagram
participant Admin as 管理后台
participant API as API 层
participant DB as 数据库层
participant Public as 公开页面
Admin->>API : PUT /api/memos/ : id/pin
API->>DB : UPDATE memos SET pinned_at = ?
DB-->>API : 更新后的备忘录
API-->>Admin : 返回更新结果
Note over Admin,DB : 数据库层执行排序
Public->>API : GET /api/memos
API->>DB : SELECT * FROM memos ORDER BY pinned_at DESC, created_at DESC
DB-->>API : 排序后的备忘录列表
API-->>Public : 返回置顶优先的列表
```

**图表来源**
- [memos.ts:201-220](file://src/api/memos.ts#L201-L220)
- [db.ts:161-164](file://src/db.ts#L161-L164)

## 详细组件分析

### 管理后台实现

管理后台的 MemoCard 组件提供了完整的置顶功能：

```mermaid
flowchart TD
Start([用户点击置顶按钮]) --> CheckState{"检查当前状态"}
CheckState --> |已置顶| CancelPin["调用取消置顶"]
CheckState --> |未置顶| SetPin["调用置顶"]
CancelPin --> APICall1["调用 API /api/memos/:id/pin"]
SetPin --> APICall2["调用 API /api/memos/:id/pin"]
APICall1 --> UpdateUI1["更新本地状态"]
APICall2 --> UpdateUI2["更新本地状态"]
UpdateUI1 --> Reload["重新加载备忘录列表"]
UpdateUI2 --> Reload
Reload --> End([完成])
```

**图表来源**
- [MemoCard.ts:108-115](file://src/frontend/admin/components/MemoCard.ts#L108-L115)
- [memo.ts:82-92](file://src/frontend/admin/actions/memo.ts#L82-L92)

#### 置顶状态标记

```mermaid
flowchart TD
CheckPin{"检查 pinned_at 是否存在"}
CheckPin --> |存在| ShowBadge["显示置顶徽章"]
CheckPin --> |不存在| HideBadge["隐藏徽章"]
ShowBadge --> BadgeStyle["应用橙色样式<br/>#e67e22"]
BadgeStyle --> PinIcon["显示 📌 图标"]
HideBadge --> NoAction["无操作"]
```

**图表来源**
- [MemoCard.ts:78-86](file://src/frontend/admin/components/MemoCard.ts#L78-L86)

**章节来源**
- [MemoCard.ts:61-346](file://src/frontend/admin/components/MemoCard.ts#L61-L346)
- [memo.ts:82-92](file://src/frontend/admin/actions/memo.ts#L82-L92)

### 瀑布流公开页实现

瀑布流公开页实现了只读的置顶状态显示：

```mermaid
classDiagram
class MasonryCard {
+div pinBadge
+div cardText
+div cardInfo
+div cardBtnGroup
+render() void
}
class CardState {
+number id
+string text
+PreparedText prepared
+string updatedAt
+string|null pinnedAt
+string rawText
}
MasonryCard --> CardState : "使用"
note for MasonryCard "仅显示置顶状态<br/>不提供操作按钮"
```

**图表来源**
- [components.ts:132-201](file://src/frontend/masonry/components.ts#L132-L201)
- [state.ts:13-20](file://src/frontend/masonry/state.ts#L13-L20)

**章节来源**
- [components.ts:132-201](file://src/frontend/masonry/components.ts#L132-L201)
- [state.ts:13-20](file://src/frontend/masonry/state.ts#L13-L20)

## 数据库设计

### 表结构扩展

数据库采用了最小侵入式的迁移策略，在现有 memos 表中新增 pinned_at 列：

```mermaid
erDiagram
MEMOS {
INTEGER id PK
TEXT content
TEXT tags
INTEGER is_public
TEXT pinned_at
TEXT created_at
TEXT updated_at
}
MEMOS ||--o{ MEMO_EMBEDDINGS : "关联"
note for MEMOS "新增 pinned_at 列<br/>NULL 表示未置顶<br/>ISO 8601 时间戳表示置顶时间"
```

**图表来源**
- [db.ts:18-28](file://src/db.ts#L18-L28)
- [db.ts:64-72](file://src/db.ts#L64-L72)

### 索引优化

为了提升查询性能，为 pinned_at 列建立了专门索引：

**章节来源**
- [db.ts:30-30](file://src/db.ts#L30-L30)

## API 设计

### 新增端点

系统新增了独立的置顶操作端点：

```mermaid
sequenceDiagram
participant Client as 客户端
participant Auth as 认证中间件
participant Handler as 处理函数
participant DB as 数据库
Client->>Handler : PUT /api/memos/ : id/pin
Handler->>Auth : 验证身份
Auth-->>Handler : 认证成功
alt pinned = true
Handler->>DB : UPDATE memos SET pinned_at = datetime('now')
else pinned = false
Handler->>DB : UPDATE memos SET pinned_at = NULL
end
DB-->>Handler : 返回更新后的备忘录
Handler-->>Client : 200 OK { memo }
```

**图表来源**
- [memos.ts:201-220](file://src/api/memos.ts#L201-L220)
- [db.ts:236-244](file://src/db.ts#L236-L244)

### 错误处理

API 层实现了完善的错误处理机制：

| 错误场景 | HTTP 状态码 | 响应内容 | 处理方式 |
|---------|------------|---------|---------|
| 无效 JSON | 400 | `{ error: "Invalid JSON" }` | 前端显示格式错误 |
| 缺少 pinned 参数 | 400 | `{ error: "pinned must be a boolean" }` | 前端验证参数类型 |
| 备忘录不存在 | 404 | `{ error: "Memo not found" }` | 前端提示操作失败 |
| 未认证访问 | 401 | 重定向到登录页 | 使用现有认证中间件 |

**章节来源**
- [memos.ts:201-220](file://src/api/memos.ts#L201-L220)

## 前端实现

### SVG 图标系统

新增了专门的图钉图标，与现有图标风格保持一致：

```mermaid
classDiagram
class SvgHelper {
+svgPin() HTMLElement
+svgLock() HTMLElement
+svgUnlock() HTMLElement
+svgEdit() HTMLElement
+svgTrash() HTMLElement
}
class PinIcon {
+width : 16
+height : 16
+viewBox : "0 0 24 24"
+stroke : "currentColor"
+stroke-width : 2
+stroke-linecap : "round"
+stroke-linejoin : "round"
}
SvgHelper --> PinIcon : "创建"
```

**图表来源**
- [svgHelper.ts:48-52](file://src/helper/svgHelper.ts#L48-L52)

### 状态管理

管理后台使用 VanJS 的响应式状态系统管理置顶状态：

```mermaid
stateDiagram-v2
[*] --> Idle
Idle --> Loading : 触发置顶操作
Loading --> Success : 操作成功
Loading --> Error : 操作失败
Success --> Idle : 更新完成
Error --> Idle : 显示错误
```

**章节来源**
- [memo.ts:82-92](file://src/frontend/admin/actions/memo.ts#L82-L92)
- [svgHelper.ts:48-52](file://src/helper/svgHelper.ts#L48-L52)

## 排序算法

### 多级排序逻辑

系统实现了复杂的多级排序算法，确保置顶备忘录始终优先显示：

```mermaid
flowchart TD
Start([获取备忘录列表]) --> CheckPin{"pinned_at 是否存在"}
CheckPin --> |存在| PinPriority["置顶优先级<br/>pinned_at IS NOT NULL = true"]
CheckPin --> |不存在| NormalOrder["普通排序<br/>按 created_at 排序"]
PinPriority --> SortByTime["按置顶时间倒序"]
SortByTime --> Combine["组合排序结果"]
NormalOrder --> Combine
Combine --> FinalResult["最终排序结果<br/>置顶 > 置顶时间倒序 > 创建时间倒序"]
FinalResult --> End([返回给前端])
```

**图表来源**
- [db.ts:161-161](file://src/db.ts#L161-L161)

### SQL 排序实现

排序逻辑通过单一 SQL 查询实现，避免了客户端排序的性能问题：

**章节来源**
- [db.ts:161-164](file://src/db.ts#L161-L164)

## 性能考虑

### 数据库优化

1. **索引优化**：为 pinned_at 列建立索引，提升排序查询性能
2. **单一查询**：通过 ORDER BY 子句实现复杂排序，减少往返次数
3. **内存效率**：使用 SQLite 的 WAL 模式，提升并发性能

### 前端优化

1. **响应式更新**：使用 VanJS 的响应式系统，只更新受影响的组件
2. **缓存策略**：瀑布流页面使用文本预排版缓存，提升渲染性能
3. **懒加载**：瀑布流采用虚拟滚动，只渲染可视区域内的卡片

## 故障排除指南

### 常见问题

| 问题描述 | 可能原因 | 解决方案 |
|---------|---------|---------|
| 置顶状态不显示 | 数据库迁移未完成 | 检查 pinned_at 列是否存在 |
| 置顶操作失败 | 权限不足 | 确认管理员身份认证 |
| 排序异常 | SQL 查询错误 | 检查 ORDER BY 语法 |
| 前端不更新 | 状态未刷新 | 调用 loadMemos() 重新加载 |

### 调试建议

1. **后端调试**：检查 API 日志和数据库查询
2. **前端调试**：使用浏览器开发者工具监控网络请求
3. **数据库调试**：直接查询 memos 表验证数据状态

**章节来源**
- [memos.ts:201-220](file://src/api/memos.ts#L201-L220)
- [db.ts:161-164](file://src/db.ts#L161-L164)

## 总结

备忘录固定功能通过精心设计的架构实现了以下目标：

### 功能特性
- **统一状态管理**：Admin 后台和公开页面共享相同的置顶状态
- **直观的用户界面**：清晰的状态标记和便捷的操作按钮
- **高性能实现**：通过数据库层面的排序优化，确保良好的用户体验

### 技术优势
- **最小侵入性**：仅新增一列，不影响现有功能
- **向后兼容**：新功能不影响现有数据和 API
- **可扩展性**：为未来可能的功能增强预留空间

### 用户体验
- **即时反馈**：置顶操作后立即反映在界面上
- **一致性**：Admin 后台和公开页面的行为完全一致
- **易用性**：简洁的图标和明确的视觉层次

该设计规格文档为备忘录固定功能的实现提供了完整的技术指导，确保功能的正确性和可维护性。