...
## 更新摘要
**变更内容**
- 新增创意时间线侧边栏组件，支持按年月分组显示创意内容
- 完善创意内容管理界面，包含提示词编辑、内容生成和流式输出
- 扩展时间线导航功能，为创意内容提供独立的时间线侧边栏
- 增强实时流式内容生成支持，提供更好的用户体验
- 新增上下文预览面板，支持预览创意内容生成时使用的备忘录上下文
- 改进创意内容生成界面，包含实时预览和错误处理机制
- 增强用户交互体验，包括流式内容生成的性能优化
...
**更新** 新增创意内容管理系统，支持提示词管理、实时流式内容生成、上下文预览和创意内容时间线导航。
...
AI_STATE["admin/ai-state.ts<br/>AI模型状态"]
...
**更新** 新增创意内容管理组件，支持提示词编辑、实时流式内容生成、上下文预览和错误处理。
...
participant Creative as "创意内容模块(creative.ts)"
...
Util-->>Admin : 备忘录列表
Creative->>Util : 加载提示词
Util->>API : GET /api/creative/prompts
API->>DB : 查询提示词
DB-->>API : 提示词列表
API-->>Util : { prompts }
Util-->>Creative : 提示词列表
Creative->>Util : 预览上下文
Util->>API : POST /api/creative/preview-context
API->>DB : 查询备忘录
API-->>Util : 上下文备忘录列表
Creative->>Util : 生成创意内容
Util->>API : POST /api/creative/generate
API->>API : SSE流式生成
API-->>Util : 流式数据
Util-->>Creative : 实时内容
```
...
- [src/admin/creative.ts:44-57](file://src/admin/creative.ts#L44-L57)
- [src/util.ts:9-20](file://src/util.ts#L9-L20)
...
**更新** 新增创意内容时间线侧边栏，具有相同的功能特性，支持创意内容的按年月分组和滚动定位。
...
- [src/admin/app.ts:428-446](file://src/admin/app.ts#L428-L446)
- [src/admin/app.ts:421-426](file://src/admin/app.ts#L421-L426)
...
**更新** 新增完整的创意内容管理界面，包括提示词编辑、内容生成、流式输出和上下文预览功能。
...
Modal->>Preview : 预览上下文
Preview->>API : POST /api/creative/preview-context
API->>DB : 查询备忘录
DB-->>API : 上下文备忘录
API-->>Preview : 上下文列表
Preview-->>User : 显示上下文预览
User->>Modal : 点击生成
...
- [src/api/creative.ts:118-247](file://src/api/creative.ts#L118-L247)
- [src/admin/creative.ts:389-567](file://src/admin/creative.ts#L389-L567)
- [src/admin/creative.ts:166-213](file://src/admin/creative.ts#L166-L213)

**章节来源**
- [src/admin/creative.ts:44-137](file://src/admin/creative.ts#L44-L137)
- [src/admin/creative.ts:145-271](file://src/admin/creative.ts#L145-L271)
- [src/admin/creative.ts:389-567](file://src/admin/creative.ts#L389-L567)
- [src/api/creative.ts:118-247](file://src/api/creative.ts#L118-L247)
...
**更新** 新增创意内容时间线侧边栏组件，支持创意内容的按年月分组导航，以及上下文预览面板组件。
...
**更新** 新增创意内容 API 通信协议，支持提示词管理、流式内容生成和上下文预览。
...
**更新** 新增 AI 模型状态共享模块，支持创意内容与备忘录编辑的 AI 功能集成。
...
- 前端性能
  ...
  - **新增** 创意内容流式渲染使用 requestAnimationFrame 优化性能。
...
**更新** 新增创意内容管理系统，提供完整的提示词管理、实时流式内容生成功能和上下文预览，进一步增强了系统的实用性和用户体验。
...
- 自定义扩展方法
  ...
  - **新增** 创意内容扩展：在 creative.ts 中添加新的提示词类型，或扩展流式生成逻辑。
  - **新增** 上下文预览扩展：在 creative.ts 中添加新的上下文预览模式，或扩展预览逻辑。